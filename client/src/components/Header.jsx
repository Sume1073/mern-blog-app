...
>
  ...
  Blog
      ...
      {/* Search Input */}
 {/* Professional Yet Cool Search Input */}
...
>
  ...
  <button 
    type="button" 
    className="relative inline-flex items-center justify-center p-0.5 mb-2 me-2 overflow-hidden text-sm font-medium text-gray-900 rounded-lg group bg-gradient-to-br from-purple-600 to-blue-500 group-hover:from-purple-600 group-hover:to-blue-500 hover:text-white dark:text-white focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800 ml-4 mt-2" // Add margin-top here
  >
    <span className="items-center flex justify-center bg-white text-gray-900 transition-all duration-75 ease-in group-enabled:group-hover:bg-opacity-0 group-enabled:group-hover:text-inherit dark:bg-gray-900 dark:text-white w-full rounded-md text-sm px-4 py-2 border border-transparent">
      ...
      <Navbar.Toggle className='lg:hidden' onClick={() => setIsNavbarOpen(!isNavbarOpen)} />
      <Navbar.Collapse className={`${isNavbarOpen ? 'block' : 'hidden'} lg:flex`}>
  <Navbar.Link
    active={path === '/'}
    as={'div'}
    className={`${
